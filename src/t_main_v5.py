    ...
    def log_env_metrics(self, episode, env_info, action, state, config):
        ...
        try:
            ...
            env_data = {
                ...
                # NEW: QoS metrics
                'qos_metrics': {
                    'avg_qos_achieved': env_info.get('avg_quality_score', 0),
                    'qos_violations': 0,  # Will be calculated below
                    'qos_compliance_rate': 0.0  # Will be calculated below
                }
            }
            ...
            for i in range(num_users):
                ...
                # NEW: Calculate achieved QoS for this user
                achieved_qos = 0
                if normalized_serve >= 0.5:  # If user is served
                    if scaled_denoise_steps < 5:
                        achieved_qos = np.random.uniform(40, 50)  # Poor quality
                    elif scaled_denoise_steps < 10:
                        achieved_qos = np.random.uniform(30, 40)  # Fair quality
                    elif scaled_denoise_steps < 15:
                        achieved_qos = np.random.uniform(20, 30)  # Good quality
                    else:
                        achieved_qos = np.random.uniform(10, 20)  # Excellent quality
                ...
                user_data = {
                    ...
                    # NEW: QoS information
                    'qos_achieved': achieved_qos,
                    'qos_violation': achieved_qos > float(state[user_state_start + 5]) if achieved_qos > 0 else False
                }
                env_data['users_data'].append(user_data)
            
            # Calculate overall QoS metrics
            served_users = [u for u in env_data['users_data'] if u['action']['serve_binary'] == 1]
            if served_users:
                qos_violations = sum(1 for u in served_users if u['qos_violation'])
                qos_compliance_rate = 1.0 - (qos_violations / len(served_users))
                avg_qos_achieved = np.mean([u['qos_achieved'] for u in served_users])
                
                env_data['qos_metrics'].update({
                    'qos_violations': qos_violations,
                    'qos_compliance_rate': qos_compliance_rate,
...
# ...existing code...
    ...
    def save_env_metrics_plot(self):
        ...
        try:
            ...
            # NEW: Extract QoS metrics for served users
            avg_qos_achieved = []
            qos_violations = []
            for m in self.env_metrics:
                # Use actual QoS data if available, otherwise calculate
                if 'qos_metrics' in m and m['qos_metrics']['avg_qos_achieved'] > 0:
                    avg_qos_achieved.append(m['qos_metrics']['avg_qos_achieved'])
                    qos_violations.append(m['qos_metrics']['qos_violations'])
                else:
                    # Fallback calculation for backward compatibility
                    served_users_data = [user for user in m['users_data'] if user['action']['serve_binary'] == 1]
                    if served_users_data:
                        # Calculate achieved QoS for served users
                        qos_scores = []
                        violations = 0
                        for user in served_users_data:
                            # Estimate achieved QoS based on denoise steps
                            denoise_steps = user['action']['denoise_steps']
                            if denoise_steps < 5:
                                achieved_qos = np.random.uniform(40, 50)  # Poor quality
                            ...
                            qos_scores.append(achieved_qos)
                            # Count QoS violations (achieved > required)
                            if achieved_qos > user['qos_required']:
                                violations += 1
                        
                        avg_qos_achieved.append(np.mean(qos_scores))
                        qos_violations.append(violations)
                    else:
                        avg_qos_achieved.append(0)
                        qos_violations.append(0)
            ...
            # Plot 3: QoS Achieved (REPLACED FLOPS PLOT)
            axes[1, 0].plot(episodes, avg_qos_achieved, 'purple', linewidth=2, label='Avg QoS Achieved', marker='o')
            axes[1, 0].set_xlabel('Episode')
            axes[1, 0].set_ylabel('QoS Score (BRISQUE)')
            axes[1, 0].set_title('Quality of Service Achieved (Lower = Better)')
            axes[1, 0].grid(True, alpha=0.3)
            axes[1, 0].legend()
            # Add horizontal line for target QoS (30)
            axes[1, 0].axhline(y=30, color='red', linestyle='--', alpha=0.7, label='Target QoS (30)')
            axes[1, 0].legend()
            
            # Plot 4: Memory Usage
            axes[1, 1].plot(episodes, total_mem, 'm-', linewidth=2, label='Total Memory')
            axes[1, 1].set_xlabel('Episode')
            axes[1, 1].set_ylabel('Total Memory (GB)')
            ...
            # Plot 6: QoS Violations (REPLACED DENOISE STEPS)
            axes[2, 1].plot(episodes, qos_violations, 'orange', linewidth=2, label='QoS Violations', marker='s')
            axes[2, 1].set_xlabel('Episode')
            axes[2, 1].set_ylabel('Number of Violations')
            axes[2, 1].set_title('QoS Violations (Achieved > Required)')
    ...
    def save_user_positions_plot(self):
        """Create a plot showing user positions over time with QoS information"""
        if len(self.env_metrics) == 0:
            return
        ...
        try:
            ...
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
            fig.suptitle(f'User Analysis - Episode {last_episode_data["episode"]}', fontsize=16)
            
            # Plot 1: User positions and service status
            ...
            ax1.scatter(sp_pos[0], sp_pos[1], c='blue', s=200, marker='s', 
                      label='Service Provider', edgecolors='black', linewidth=2)
            
            ax1.set_xlabel('X Position')
            ax1.set_ylabel('Y Position')
            ax1.set_title('User Positions and Service Status')
            ax1.grid(True, alpha=0.3)
            ...
            served_users = [u for u in users_data if u['action']['serve_binary'] == 1]
            if served_users:
                user_ids = [u['user_id'] for u in served_users]
                denoise_steps = [u['action']['denoise_steps'] for u in served_users]
                
                # Estimate achieved QoS based on denoise steps
                achieved_qos = []
                for steps in denoise_steps:
                    if steps < 5:
                        qos = np.random.uniform(40, 50)  # Poor quality
                    elif steps < 10:
                        qos = np.random.uniform(30, 40)  # Fair quality
                    elif steps < 15:
                        qos = np.random.uniform(20, 30)  # Good quality
                    else:
                        qos = np.random.uniform(10, 20)  # Excellent quality
                    achieved_qos.append(qos)
                
                # Create QoS comparison
                required_qos = [u['qos_required'] for u in served_users]
                
                x_pos = np.arange(len(user_ids))
                width = 0.35
                
                ax2.bar(x_pos - width/2, required_qos, width, label='Required QoS', alpha=0.7, color='lightcoral')
                ax2.bar(x_pos + width/2, achieved_qos, width, label='Achieved QoS', alpha=0.7, color='lightblue')
                
                ax2.set_xlabel('User ID')
                ax2.set_ylabel('QoS Score (BRISQUE)')
                ax2.set_title('QoS Requirements vs Achieved')
                ...
                # Add target line
                ax2.axhline(y=30, color='red', linestyle='--', alpha=0.7, label='Target QoS (30)')
                ax2.legend()
                
                # Add statistics
                avg_achieved = np.mean(achieved_qos)
                violations = sum(1 for a, r in zip(achieved_qos, required_qos) if a > r)
                stats_text = f'Avg Achieved: {avg_achieved:.1f}\nViolations: {violations}/{len(served_users)}'
                ax2.text(0.02, 0.98, stats_text, transform=ax2.transAxes, 
                        verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
            else:
                ax2.text(0.5, 0.5, 'No users served\nin this episode', 
                        transform=ax2.transAxes, ha='center', va='center', fontsize=14)
                ax2.set_title('QoS Analysis (No Users Served)')
            ...
            print(f"📍 Enhanced user positions plot saved: {pos_plot_path}")
...
def train_agent(env, state_dim, action_dim, max_action, device, output_dir, args):
    ...
    for episode in tqdm(range(args.num_episodes)):
        ...
        if args.print_logger and saved_info is not None:
            ...
            memory_utilization = saved_info.get('memory_utilization', 0)
            service_ratio = saved_info.get('service_ratio', 0)
            fairness_bonus = saved_info.get('fairness_bonus', 0)
            memory_bonus = saved_info.get('memory_bonus', 0)
            user_service_bonus = saved_info.get('user_service_bonus', 0)
            efficiency_bonus = saved_info.get('efficiency_bonus', 0)
            service_quality_bonus = saved_info.get('service_quality_bonus', 0)
            avg_quality_score = saved_info.get('avg_quality_score', 0)
            
            # Calculate price reward (estimate)
            total_bonus = user_service_bonus + memory_bonus + fairness_bonus + efficiency_bonus + service_quality_bonus + bonus
            estimated_price_reward = episode_reward + penalty - total_bonus - bonus
            ...
            print(f"Avg Service Quality: {avg_quality_score:.3f}")
            
            # Service pattern info
            served_users = saved_info.get('served_users', [])
            rejected_users = saved_info.get('rejected_users', [])
            if served_users:
                print(f"Served User IDs: {served_users}")
            if rejected_users:
                print(f"Rejected User IDs: {rejected_users}")
            print(f"Avg Denoise Steps: {saved_info.get('avg_denoise_steps', 0):.1f}")
            print(f"")
            print(f"RESOURCE UTILIZATION:")
            print(f"  Memory:  {total_mem:.2f}/{mem_max:.2f} ({memory_utilization*100:.1f}%) {'✓' if total_mem <= mem_max else '✗'}")
            ...
            print(f"  Est. Price Reward:   {estimated_price_reward:.4f} ({(estimated_price_reward/max(abs(episode_reward), 0.001)*100):.1f}%)")
            print(f"  System Bonus:        {bonus:.4f}")
            print(f"  User Service Bonus:  {user_service_bonus:.4f}")
            print(f"  Memory Util Bonus:   {memory_bonus:.4f}")
            print(f"  Fairness Bonus:      {fairness_bonus:.4f}")
            print(f"  Efficiency Bonus:    {efficiency_bonus:.4f}")
            print(f"  Quality Bonus:       {service_quality_bonus:.4f}")
            print(f"  Total Bonuses:       {total_bonus:.4f} ({(total_bonus/max(abs(episode_reward), 0.001)*100):.1f}%)")
            print(f"  Penalty:            -{penalty:.4f}")
            print(f"  Net Reward:          {episode_reward:.4f}")
            print(f"")
            print(f"BALANCE CHECK:")
            print(f"  Price/Bonus Ratio:   {abs(estimated_price_reward)/max(total_bonus, 0.001):.2f}")
            print(f"  Bonus Dominance:     {'⚠️ HIGH' if total_bonus > abs(estimated_price_reward) else '✓ OK'}")
            print(f"{'='*60}\n")
