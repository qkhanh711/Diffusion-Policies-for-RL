...
class GAIServiceEnv_v1(gym.Env):
    ...
    def _compute_reward(self, action):
        ...
        for i, user in enumerate(self.users):
            ...
            denoise_steps = np.random.randint(1, self.config["max_denoise_steps"] + 1)
            # print(f"User {i}: serve={serve}, denoise_steps={denoise_steps}")
            ...
            if serve:
                ...
                total_flops += flops
                total_mem += mem
            # print(total_mem, total_flops, total_latency)
            # print(self.config["Mmax"], self.config["Gmax"], user.qos_required)
