...
class GAIServiceEnv_v1(gym.Env):
    ...
    def _compute_reward(self, action):
        ...
        # IMPORTANT: Memory resets each step - previous users have finished service
        # Each step represents a new batch of service requests
        current_memory = 0
        ...
        for i, user in enumerate(self.users):
            ...
            if serve:
                # Scale denoise steps from action space [1, max_denoise_steps] 
                # Assuming action is already in correct range, but clip to be safe
                ...
                if current_memory + mem <= self.config["Mmax"]:
                    ...
                    total_flops += flops
                    current_memory += mem  # Add to current step memory usage
                    served += 1
                else:
                    # Cannot serve this user due to memory constraint in this step
                    # User is rejected for this step only
                    pass

        # Set total memory for this step
        total_mem = current_memory

        # System-level constraint penalties for this step
