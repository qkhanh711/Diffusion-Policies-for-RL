    ...
    def train(self, replay_buffer, iterations, batch_size=100, log_writer=None):
        ...
        for iteration in range(iterations):
            try:
                ...
                """ Efficient Value Function Training """
                with torch.no_grad():
                    next_values = self.critic(next_state).squeeze(-1)  # More efficient squeeze
                    returns = reward.squeeze(-1) + self.gamma * next_values * not_done.squeeze(-1)
                
                # Single value function update (instead of 3x)
                ...
                with torch.no_grad():
                    ...
                    # Normalize advantages efficiently
                    if advantages.std() > 1e-8:
                        advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-8)
                    
                    # Find positive advantages
                    positive_mask = advantages > 0
                
                if positive_mask.sum() > 0:
                    # Use indexing instead of creating new tensors
                    positive_indices = positive_mask.nonzero(as_tuple=True)[0]
                    
                    # Efficient subset selection
                    good_advantages = advantages[positive_indices]
                    
                    # Compute weights once
                    weights = torch.softmax(good_advantages, dim=0).detach()
                    
                    # Efficient diffusion loss computation
                    bc_losses = self.actor.loss(action[positive_indices], state[positive_indices])
                    
                    # Weighted loss with reduced regularization
                    policy_loss = (bc_losses * weights).mean()
                ...
                # Less frequent EMA updates for speed
                if self.step % (self.update_ema_every * 2) == 0:  # 2x less frequent
                    self.step_ema()

                self.step += 1

                # Efficient logging with minimal GPU->CPU transfers
                metric['ppo_loss'].append(policy_loss.item())
                metric['value_loss'].append(value_loss.item())
                metric['actor_loss'].append(total_actor_loss.item())

            except Exception as e:
                print(f"Error in PPO training iteration {iteration}: {e}")
                # Use device-appropriate tensors
                zero_loss = torch.zeros(1, device=device)
                metric['ppo_loss'].append(zero_loss.item())
                metric['value_loss'].append(zero_loss.item())
                metric['actor_loss'].append(zero_loss.item())
                continue

        # Much less frequent learning rate scheduling
        if self.lr_decay and self.step % 500 == 0:  # 2.5x less frequent
