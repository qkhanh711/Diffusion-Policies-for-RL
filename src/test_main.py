...
class MetricsLogger:
        ...
        try:
            ...
            for i in range(num_users):
                ...
                user_data = {
                    ...
                    'action': {
                        'serve_decision': float(action[2*i]),
                        'serve_binary': 1 if action[2*i] >= 0.5 else 0,
                        'denoise_steps': int(np.clip(action[2*i + 1], 1, config["max_denoise_steps"]))
                    }
                }
    ...
    def create_user_movement_gif(self, episode_data, filename_suffix=""):
        ...
        try:
            ...
            # Save as GIF
            gif_filename = f"user_movement_episode_{episode_data['episode']}{filename_suffix}.gif"
    ...
    def create_metrics_progression_gif(self, episode_data, filename_suffix=""):
        ...
        try:
            ...
            # Save as GIF
            gif_filename = f"metrics_progression_episode_{episode_data['episode']}{filename_suffix}.gif"
            gif_path = os.path.join(self.save_dir, gif_filename)
    ...
    def create_combined_episode_gif(self, episode_data, filename_suffix=""):
        ...
        try:
            ...
            # Save as GIF
            gif_filename = f"combined_episode_{episode_data['episode']}{filename_suffix}.gif"
            gif_path = os.path.join(self.save_dir, gif_filename)
    ...
    def save_to_csv(self):
        ...
        if self.env_metrics:
            # Flatten environment metrics for CSV
            flattened_env_metrics = []
            for env_data in self.env_metrics:
                base_row = {
                    ...
                    'latency_ok': env_data['constraints_status']['latency_ok'],
                    'flops_ok': env_data['constraints_status']['flops_ok'],
                    'mem_ok': env_data['constraints_status']['mem_ok']
                }
                ...
                for user in env_data['users_data']:
                    ...
                    flattened_env_metrics.append(user_row)
            
            env_file = os.path.join(self.save_dir, f"environment_metrics.csv")
            if flattened_env_metrics:
                with open(env_file, 'w', newline='') as f:
                    writer = csv.DictWriter(f, fieldnames=flattened_env_metrics[0].keys())
                    writer.writeheader()
                    writer.writerows(flattened_env_metrics)
                print(f"Environment metrics saved to: {env_file}")
...
def train_agent(env, state_dim, action_dim, max_action, device, output_dir, args):
    ...
    for episode in tqdm(range(args.num_episodes)):
        ...
        # Start recording episode for GIF creation
        should_record_gif = (episode % 20 == 0) or (episode < 5) or (episode >= args.num_episodes - 5)
        ...
        if (episode + 1) % 10 == 0:
            ...
            metrics_logger.save_env_metrics_plot()  # Save environment metrics plot
            
        if (episode + 1) % 50 == 0:
            metrics_logger.save_user_positions_plot()  # Save user positions plot
            # Create GIFs for recent episodes
            metrics_logger.save_episode_gifs()
