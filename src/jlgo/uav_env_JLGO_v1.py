...
class UAVGenAIEnv(gym.Env):
    ...
    def __init__(self, config=None, num_ues=3, total_timesteps=50, output_gen_path="./data/output/generated_images/", config_path="config.yaml"):
        ...
        # UE Configuration
        self.data_size_per_ue = get_config_value_from_config(CONFIG, 'ue.data_size_per_ue', 100000)  # Reduced from 1MB to 100KB
        ...
        self.enable_qos_printing = get_config_value_from_config(CONFIG, 'environment.enable_qos_printing', True)
        self.qos_print_interval = get_config_value_from_config(CONFIG, 'environment.qos_print_interval', 10)
        print(f"📊 QoS Monitoring: {'Enabled' if self.enable_qos_printing else 'Disabled'} (interval: {self.qos_print_interval} steps)")
    ...
    def print_qos_metrics(self, step_interval=None):
        """Print comprehensive QoS metrics in an organized format"""
        if not self.enable_qos_printing:
            return
            
        interval = step_interval if step_interval is not None else self.qos_print_interval
        if self.current_step % interval != 0:
            return
            
        print("\n" + "="*80)
        print(f"📊 QoS METRICS - Step {self.current_step}/{self.total_timesteps}")
        print("="*80)
        
        # Basic completion status
        completion_rate = np.sum(self.ue_data_received) / self.num_ues * 100
        print(f"🎯 Overall Completion Rate: {completion_rate:.1f}% ({np.sum(self.ue_data_received)}/{self.num_ues} UEs)")
        
        # Data flow progress
        sent_rate = np.sum(self.ue_data_sent) / self.num_ues * 100
        processed_rate = np.sum(self.ue_data_processed) / self.num_ues * 100
        print(f"📤 Upload Progress: {sent_rate:.1f}% | 🔄 Processing: {processed_rate:.1f}% | 📥 Download: {completion_rate:.1f}%")
        ...
        for ue_idx in range(self.num_ues):
            task_type = self.ue_task_types[ue_idx] if hasattr(self, 'ue_task_types') else 'genai'
            status = "✅" if self.ue_data_received[ue_idx] else "🔄" if self.ue_data_processed[ue_idx] else "📤" if self.ue_data_sent[ue_idx] else "⏳"
            ...
            if self.ue_data_received[ue_idx]:
                ...
                quality_gap = achieved_quality - target_quality
                total_quality_gap += abs(quality_gap)
                ...
                if achieved_quality <= target_quality:
                    ...
                    quality_status = f"❌ MISS (+{quality_gap:.1f})"
                
                print(f"  UE{ue_idx} ({task_type}): {status} Quality: {achieved_quality:.1f}/{target_quality:.1f} {quality_status} | Steps: {self.current_denoising_steps[ue_idx]:.0f}")
            else:
                print(f"  UE{ue_idx} ({task_type}): {status} Target: {self.target_quality_scores[ue_idx]:.1f} | Steps: {self.current_denoising_steps[ue_idx]:.0f}")
        
        # Quality summary
        if np.sum(self.ue_data_received) > 0:
            avg_quality_gap = total_quality_gap / np.sum(self.ue_data_received)
            quality_success_rate = quality_met_count / np.sum(self.ue_data_received) * 100
            print(f"  📈 Quality Success Rate: {quality_success_rate:.1f}% | Avg Gap: {avg_quality_gap:.2f}")
        
        # Processing efficiency
        print(f"\n⚡ PROCESSING EFFICIENCY:")
        if self.current_processing_ue >= 0:
            progress = (self.processing_start_time - self.processing_remaining_time) / max(self.processing_start_time, 0.1) * 100
            print(f"  🔄 Currently Processing: UE{self.current_processing_ue} ({progress:.1f}% complete)")
            print(f"  ⏰ Remaining Time: {self.processing_remaining_time:.1f}s")
        else:
            print(f"  💤 BS Idle | Queue Length: {len(self.processing_queue)}")
        
        print(f"  📊 Total BS Processing Time: {self.bs_total_processing_time:.1f}s")
        if np.sum(self.ue_data_processed) > 0:
            avg_processing_time = self.bs_total_processing_time / np.sum(self.ue_data_processed)
            print(f"  📊 Average Processing Time: {avg_processing_time:.1f}s per UE")
        
        # Communication status
        print(f"\n📡 COMMUNICATION STATUS:")
        uav_bs_connected = self._can_communicate(self.uav_current_loc, self.bs_loc)
        uav_bs_dist = self._calculate_distance(self.uav_current_loc, self.bs_loc)
        print(f"  🚁↔️🏢 UAV-BS: {'✅ Connected' if uav_bs_connected else '❌ Disconnected'} (dist: {uav_bs_dist:.1f}m)")
        
        connected_ues = 0
        for ue_idx in range(self.num_ues):
            ue_connected = self._can_communicate(self.uav_current_loc, self.ue_locs[ue_idx])
            ue_dist = self._calculate_distance(self.uav_current_loc, self.ue_locs[ue_idx])
            status_icon = "✅" if ue_connected else "❌"
            if ue_connected:
                connected_ues += 1
            print(f"  🚁↔️📱UE{ue_idx}: {status_icon} (dist: {ue_dist:.1f}m)")
        
        connectivity_rate = connected_ues / self.num_ues * 100
        print(f"  📶 Overall Connectivity: {connectivity_rate:.1f}% ({connected_ues}/{self.num_ues} UEs)")
        
        # Position info
        print(f"\n📍 POSITIONING:")
        print(f"  🚁 UAV Position: ({self.uav_current_loc[0]:.1f}, {self.uav_current_loc[1]:.1f}, {self.uav_current_loc[2]:.1f})")
        print(f"  🚁 UAV Velocity: ({self.uav_velocity[0]:.1f}, {self.uav_velocity[1]:.1f}, {self.uav_velocity[2]:.1f}) m/s")
        print(f"  ✈️ Flight Mode: {self.flight_mode.upper()}")
        
        print("="*80 + "\n")

    def print_final_qos_summary(self):
        """Print final QoS summary at episode end"""
        print("\n" + "🏁"*40)
        print("FINAL QoS SUMMARY")
        print("🏁"*40)
        
        # Overall completion
        completion_rate = np.sum(self.ue_data_received) / self.num_ues * 100
        print(f"🎯 Final Completion Rate: {completion_rate:.1f}% ({np.sum(self.ue_data_received)}/{self.num_ues} UEs)")
        ...
        for ue_idx in range(self.num_ues):
            if self.ue_data_received[ue_idx]:
                achieved_quality = self._calculate_quality_score(ue_idx)
                target_quality = self.target_quality_scores[ue_idx]
                total_quality_score += achieved_quality
        ...
        if np.sum(self.ue_data_received) > 0:
            avg_quality = total_quality_score / np.sum(self.ue_data_received)
            quality_success_rate = quality_met_count / np.sum(self.ue_data_received) * 100
            print(f"🎨 Quality Success Rate: {quality_success_rate:.1f}%")
            print(f"🎨 Average Quality Score: {avg_quality:.2f}")
            print(f"⚠️ Quality Violations: {quality_violations}")
        
        # Processing efficiency
        if np.sum(self.ue_data_processed) > 0:
            avg_processing_time = self.bs_total_processing_time / np.sum(self.ue_data_processed)
            print(f"⚡ Average Processing Time: {avg_processing_time:.1f}s per UE")
            print(f"⚡ Total Processing Time: {self.bs_total_processing_time:.1f}s")
        
        # Episode efficiency
        print(f"⏱️ Episode Duration: {self.current_step} steps")
        if completion_rate == 100:
            print(f"🏆 SUCCESS: All UEs completed in {self.current_step} steps!")
        else:
            print(f"⚠️ INCOMPLETE: {100-completion_rate:.1f}% UEs did not complete")
        
        print("🏁"*40 + "\n")

    def print_qos_now(self):
        """Force print QoS metrics immediately regardless of settings"""
        old_setting = self.enable_qos_printing
        self.enable_qos_printing = True
        self.print_qos_metrics(step_interval=1)
        self.enable_qos_printing = old_setting

    def set_qos_printing(self, enabled=True, interval=10):
        """Enable/disable QoS printing and set interval"""
        self.enable_qos_printing = enabled
        self.qos_print_interval = interval
        print(f"📊 QoS Printing: {'Enabled' if enabled else 'Disabled'} (interval: {interval} steps)")
    ...
    def _get_state(self):
        ...
        for ue in range(self.num_ues):
            # Use only the attributes that are actually initialized
            state.extend([
                self.ue_image_sizes[ue], 
                self.ue_quality_requirements[ue], 
                self.current_denoising_steps[ue],
                float(self.ue_data_sent[ue]),
                float(self.ue_data_processed[ue]),
                float(self.ue_data_received[ue])
            ])
        state.extend(self.uav_current_loc.tolist())
        return np.array(state, dtype=np.float32)
    ...
    def step(self, action):
        ...
        # Print QoS metrics periodically
        self.print_qos_metrics()
        ...
        # Print final QoS summary if episode is complete
        if terminated and self.enable_qos_printing:
            self.print_final_qos_summary()
    ...
    def _process_data_flow(self):
        ...
        for ue_idx in range(self.num_ues):
            if not self.ue_data_sent[ue_idx]:
                ...
                # Debug transmission conditions
                if self.current_step % 10 == 0 and ue_idx == 0:  # Debug first UE only
                    ue_to_uav_time = self._calculate_transmission_time(self.ue_locs[ue_idx], self.uav_current_loc)
                    uav_to_bs_time = self._calculate_transmission_time(self.uav_current_loc, self.bs_loc)
                    upload_time = ue_to_uav_time + uav_to_bs_time
                    timeout = self.transmission_timeout if self.transmission_timeout else self.tau_slot_duration
                    # print(f"UE{ue_idx} transmission: ue_to_uav={ue_to_uav}, uav_to_bs={uav_to_bs}, "
                        #   f"upload_time={upload_time:.3f}s, timeout={timeout:.3f}s")
    ...
    def _calculate_paper_reward(self) -> float:
        ...
        # Progress rewards for intermediate steps (IMPORTANT for early training)
        progress_reward = 0.0
        
        # Reward for data sent (upload progress)
        progress_reward += np.sum(self.ue_data_sent) * 2.0
        
        # Reward for data processed (processing progress)  
        progress_reward += np.sum(self.ue_data_processed) * 5.0
        
        # Reward for data received (completion progress)
        progress_reward += np.sum(self.ue_data_received) * 10.0
        ...
        # R = ψ·I - λ_q·Q - λ_l·L - λ_e·E + λ_p·P + λ_c·C + progress_reward (positioning & connectivity as REWARDS)
        total_reward = (
            ...
            + progress_reward  # Progress rewards for intermediate steps
        )
        
        # Debug information (only print occasionally to avoid spam)
        # if self.current_step % 10 == 0:  # Print every 10 steps
        #     print(f"Step {self.current_step}: Progress={progress_reward:.1f}, "
        #           f"Positioning={positioning_reward:.1f}, Connectivity={connectivity_reward:.1f}, "
        #           f"Total={total_reward:.1f}, Data_sent={np.sum(self.ue_data_sent)}, "
        #           f"Data_processed={np.sum(self.ue_data_processed)}, Data_received={np.sum(self.ue_data_received)}")
...
if __name__ == "__main__":
    print("🚁" + "="*78 + "🚁")
    print("  UAV GenAI Environment Demo - Full Feature Showcase")
    print("🚁" + "="*78 + "🚁")
    
    # Initialize environment
    print("\n🔧 Initializing Environment...")
    env = UAVGenAIEnv(config=CONFIG, num_ues=3, total_timesteps=20)
    ...
    print(f"   • Number of UEs: {env.num_ues}")
    print(f"   • UAV Position: ({env.uav_current_loc[0]:.1f}, {env.uav_current_loc[1]:.1f}, {env.uav_current_loc[2]:.1f})")
    print(f"   • Flight Mode: {env.flight_mode.upper()}")
    print(f"   • Total Timesteps: {env.total_timesteps}")
    
    # Print action space design info
    print("\n🎮 Action Space Design Information:")
    action_space_info = env.get_action_space_info()
    for key, value in action_space_info.items():
    ...
    env.print_action_space_comparison()
    
    # Show UE information
    print(f"\n👥 UE Configuration:")
    for ue_idx in range(env.num_ues):
        task_type = env.ue_task_types[ue_idx] if hasattr(env, 'ue_task_types') else 'genai'
        print(f"   • UE{ue_idx} ({task_type}):")
        print(f"     - Position: ({env.ue_locs[ue_idx][0]:.1f}, {env.ue_locs[ue_idx][1]:.1f}, {env.ue_locs[ue_idx][2]:.1f})")
        print(f"     - Image Size: {env.ue_image_sizes[ue_idx]:.0f} pixels²")
        print(f"     - Quality Target: {env.target_quality_scores[ue_idx]:.1f}")
        print(f"     - Denoising Steps: {env.current_denoising_steps[ue_idx]:.0f}")
    
    # Show reward configuration
    print(f"\n🎯 Reward Configuration:")
    print(f"   • Reward Function: {'Paper-compliant' if env.use_paper_reward else 'Legacy'}")
    print(f"   • QoS Monitoring: {'Enabled' if env.enable_qos_printing else 'Disabled'}")
    print(f"   • QoS Print Interval: {env.qos_print_interval} steps")
    
    if env.use_paper_reward:
        print(f"   • Paper Reward Parameters:")
        print(f"     - λ_quality: {env.lambda_quality}")
        print(f"     - λ_latency: {env.lambda_latency}")
        print(f"     - λ_energy: {env.lambda_energy}")
        print(f"     - λ_positioning: {env.lambda_positioning}")
        print(f"     - λ_connectivity: {env.lambda_connectivity}")
        print(f"     - ψ_bonus: {env.psi_bonus}")
        print(f"     - τ_deadline: {env.tau_deadline}s")
    
    # Demo different QoS settings
    print(f"\n⚙️ Demonstrating QoS Settings:")
    print("   • Default QoS printing enabled with 10-step interval")
    
    # Force print initial QoS
    print(f"\n📊 Initial QoS Status:")
    env.print_qos_now()
    
    # Run simulation steps
    print(f"\n🚀 Starting Simulation (showing every 5 steps)...")
    print("🔄" + "-"*78 + "🔄")
    
    total_reward = 0.0
    episode_rewards = []
    
    for step in range(env.total_timesteps):
        ...
        # Print step info every 5 steps
        if (step + 1) % 5 == 0 or terminated:
            print(f"\n📈 Step {step + 1} Summary:")
            print(f"   • Reward: {reward:.2f} | Cumulative: {total_reward:.2f}")
            print(f"   • UAV Position: ({env.uav_current_loc[0]:.1f}, {env.uav_current_loc[1]:.1f}, {env.uav_current_loc[2]:.1f})")
            print(f"   • UAV Velocity: ({env.uav_velocity[0]:.1f}, {env.uav_velocity[1]:.1f}, {env.uav_velocity[2]:.1f}) m/s")
            print(f"   • Data Sent: {np.sum(env.ue_data_sent)}/{env.num_ues}")
            print(f"   • Data Processed: {np.sum(env.ue_data_processed)}/{env.num_ues}")
            print(f"   • Data Received: {np.sum(env.ue_data_received)}/{env.num_ues}")
            
            # Connectivity status
            uav_bs_connected = env._can_communicate(env.uav_current_loc, env.bs_loc)
            connected_ues = sum(1 for ue_idx in range(env.num_ues) 
                              if env._can_communicate(env.uav_current_loc, env.ue_locs[ue_idx]))
            print(f"   • Connectivity: UAV-BS: {'✅' if uav_bs_connected else '❌'} | UEs: {connected_ues}/{env.num_ues}")
            
            if env.current_processing_ue >= 0:
                progress = (env.processing_start_time - env.processing_remaining_time) / max(env.processing_start_time, 0.1) * 100
                print(f"   • Processing: UE{env.current_processing_ue} ({progress:.1f}% complete)")
            else:
                print(f"   • Processing: BS Idle | Queue: {len(env.processing_queue)}")
    ...
    completion_rate = np.sum(env.ue_data_received) / env.num_ues * 100
    print(f"\n📊 Performance Metrics:")
    print(f"   • Total Steps: {env.current_step}")
    print(f"   • Total Reward: {total_reward:.2f}")
    print(f"   • Average Reward: {total_reward/env.current_step:.2f}")
    print(f"   • Completion Rate: {completion_rate:.1f}%")
    print(f"   • Final UAV Position: ({env.uav_current_loc[0]:.1f}, {env.uav_current_loc[1]:.1f}, {env.uav_current_loc[2]:.1f})")
    
    # Data flow summary
    print(f"\n📤 Data Flow Summary:")
    print(f"   • UEs Data Sent: {np.sum(env.ue_data_sent)}/{env.num_ues}")
    print(f"   • UEs Data Processed: {np.sum(env.ue_data_processed)}/{env.num_ues}")
    print(f"   • UEs Data Received: {np.sum(env.ue_data_received)}/{env.num_ues}")
    print(f"   • Total BS Processing Time: {env.bs_total_processing_time:.1f}s")
    
    if np.sum(env.ue_data_processed) > 0:
        avg_processing_time = env.bs_total_processing_time / np.sum(env.ue_data_processed)
        print(f"   • Average Processing Time: {avg_processing_time:.1f}s per UE")
    ...
    quality_met = 0
    for ue_idx in range(env.num_ues):
        if env.ue_data_received[ue_idx]:
            achieved_quality = env._calculate_quality_score(ue_idx)
            target_quality = env.target_quality_scores[ue_idx]
            if achieved_quality <= target_quality:
                quality_met += 1
                status = "✅ MET"
            else:
                quality_violations += 1
                status = f"❌ MISS (+{achieved_quality - target_quality:.1f})"
            print(f"   • UE{ue_idx}: {achieved_quality:.1f}/{target_quality:.1f} {status}")
    
    if np.sum(env.ue_data_received) > 0:
        quality_success_rate = quality_met / np.sum(env.ue_data_received) * 100
        print(f"   • Quality Success Rate: {quality_success_rate:.1f}%")
        print(f"   • Quality Violations: {quality_violations}")
    
    # Communication summary
    final_uav_bs_connected = env._can_communicate(env.uav_current_loc, env.bs_loc)
    final_connected_ues = sum(1 for ue_idx in range(env.num_ues) 
                            if env._can_communicate(env.uav_current_loc, env.ue_locs[ue_idx]))
    
    print(f"\n📡 Final Communication Status:")
    print(f"   • UAV-BS Connection: {'✅ Connected' if final_uav_bs_connected else '❌ Disconnected'}")
    print(f"   • Connected UEs: {final_connected_ues}/{env.num_ues}")
    
    # Show reward components (if using legacy reward)
    if not env.use_paper_reward:
        print(f"\n🎯 Reward Weights Configuration:")
        weights = env.get_reward_weights()
        for key, value in weights.items():
            print(f"   • {key}: {value}")
    
    # Demo QoS feature toggle
    print(f"\n⚙️ Demonstrating QoS Feature Control:")
    print("   • Disabling QoS printing...")
    env.set_qos_printing(enabled=False)
    
    print("   • Re-enabling with 5-step interval...")
    env.set_qos_printing(enabled=True, interval=5)
    
    print("   • Force printing current QoS status...")
    env.print_qos_now()
    
    # Success/Failure assessment
    print(f"\n🏆 Mission Assessment:")
    if completion_rate == 100:
        print(f"   🎉 SUCCESS: All UEs completed their tasks!")
        print(f"   ⭐ Mission completed in {env.current_step} steps")
    elif completion_rate >= 66:
        print(f"   ✅ PARTIAL SUCCESS: {completion_rate:.1f}% completion rate")
        print(f"   💡 Room for improvement in connectivity and positioning")
    else:
        print(f"   ❌ MISSION INCOMPLETE: Only {completion_rate:.1f}% completion")
        print(f"   🔧 Need better strategy for UAV positioning and scheduling")
    
    if quality_violations == 0 and np.sum(env.ue_data_received) > 0:
        print(f"   🎨 QUALITY EXCELLENT: All completed tasks met quality requirements!")
    elif quality_violations > 0:
        print(f"   ⚠️ QUALITY ISSUES: {quality_violations} quality violations detected")
    
    # Environment info
    print(f"\n📋 Environment Information:")
    final_info = env._get_info()
    important_metrics = [
        'completion_rate', 'qos_quality_score_avg', 'energy_total_consumption',
        'latency_total_avg', 'current_processing_type'
    ]
    ...
    print(f"\n🔧 QoS Monitoring Features Demonstrated:")
    print(f"   ✅ Periodic QoS metrics display")
    print(f"   ✅ Final episode summary")
    print(f"   ✅ Real-time connectivity monitoring")
    print(f"   ✅ Quality assessment tracking")
    print(f"   ✅ Processing efficiency monitoring")
    print(f"   ✅ Configurable print intervals")
    print(f"   ✅ On-demand QoS status")
    
    print(f"\n" + "🚁" + "="*78 + "🚁")
    print("  Demo Complete - UAV GenAI Environment Ready for Training!")
    print("🚁" + "="*78 + "🚁\n")
