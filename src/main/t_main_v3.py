        ...
        try:
            ...
            step_data = {
                ...
                'system_totals': {
                    ...
                    'total_latency': info.get('total_latency', 0),  # Fixed: consistent key
                    'total_flops': info.get('total_flops', 0),
                    'total_mem': info.get('total_mem', 0)  # Fixed: consistent key
                }
            }
        ...
        try:
            ...
            env_data = {
                ...
                'system_totals': {
                    ...
                    'total_latency': env_info.get('total_latency', 0),  # Fixed: consistent key
                    'total_flops': env_info.get('total_flops', 0),
                    'total_mem': env_info.get('total_mem', 0),  # Fixed: consistent key
                    'bonus': env_info.get('bonus', 0),
                    'penalty': env_info.get('penalty', 0)
                },
                'constraints_status': {
                    'latency_ok': env_info.get('total_latency', 0) <= env_info.get('latency_limit', config["sys_tau"] * num_users),  # Fixed
                    'flops_ok': env_info.get('total_flops', 0) <= config["Gmax"],
                    'mem_ok': env_info.get('total_mem', 0) <= config["Mmax"]  # Fixed: consistent key
                }
            }
    ...
    for episode in tqdm(range(args.num_episodes)):
        ...
        while not done:
            ...
            if args.print_logger:
                ...
                resource_util = info.get('resource_utilization', {})
                
                print(f"Episode {episode}, Step {step_count}:")
                print(f"  Users: {served} served, {rejected} rejected")
                print(f"  Memory: {total_mem:.2f}/{mem_max:.2f} ({resource_util.get('memory', 0)*100:.1f}%)")
                print(f"  FLOPS: {resource_util.get('flops', 0)*100:.1f}% utilized")
                print(f"  Latency: {resource_util.get('latency', 0)*100:.1f}% utilized")
                print(f"  Service Ratio: {info.get('service_ratio', 0)*100:.1f}%")
        ...
        if args.print_logger and saved_info is not None:
            total_mem = saved_info.get('total_mem', 0)  # Fixed: consistent key
            mem_max = config.get('Mmax', 0)
            total_flops = saved_info.get('total_flops', 0)
            flops_max = config.get('Gmax', 0)
            total_latency = saved_info.get('total_latency', 0)  # Fixed: consistent key
            latency_max = saved_info.get('latency_limit', config.get('sys_tau', 0) * config.get('num_users', 1))  # Fixed
            served = saved_info.get('served', 0)
            rejected = saved_info.get('rejected', 0)
            ...
            print(f"Users Rejected: {rejected}")
            print(f"")
            print(f"RESOURCE USAGE:")
            print(f"  Memory:  {total_mem:.2f}/{mem_max:.2f} ({(total_mem/mem_max*100) if mem_max > 0 else 0:.1f}%) {'✓' if total_mem <= mem_max else '✗'}")
            print(f"  FLOPS:   {total_flops:.2f}/{flops_max:.2f} ({(total_flops/flops_max*100) if flops_max > 0 else 0:.1f}%) {'✓' if total_flops <= flops_max else '✗'}")
            print(f"  Latency: {total_latency:.2f}/{latency_max:.2f} ({(total_latency/latency_max*100) if latency_max > 0 else 0:.1f}%) {'✓' if total_latency <= latency_max else '✗'}")
            print(f"")
