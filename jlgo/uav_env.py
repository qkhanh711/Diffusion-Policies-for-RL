...
def compute_channel_gain(pos_tx, pos_rx, g0=1.42e-4, beta=2.0):
    distance = np.linalg.norm(pos_tx - pos_rx)
    return g0 / (distance ** beta + 1e-9)  # tránh chia 0

def compute_energy(ue, uav_pos, bs_pos, D_i):
    d_ue_uav = np.linalg.norm(uav_pos - ue.pos)
    ...
    if ue.w == 1:
        ...
        E_proc = 0.05 * ue.K
    return phi * (d_ue_uav + d_uav_bs) + E_proc

def compute_latency(ue, D_i):
    ...
    if ue.w == 1:
        ...
        L_proc = 0.005 * ue.K
    return L_comm + L_proc

def compute_qos(ue, D_i):
    if ue.w == 1:
        if D_i <= 10:
            return np.random.randint(38, 45)
        elif D_i <= 20:
            return np.random.randint(25, 38)
        elif D_i <= 30:
        ...
        return 100.0  # giả định BRISQUE cho DNN không ý nghĩa
...
class UE:
    ...
    def reset(self):
        ...
        self.Simg = np.random.randint(128, 1024) * 1024
        self.Sprompt = np.random.randint(1, 10) * 1024 if self.w == 1 else 0
        self.f = self.config.get('f_inf_BS', 1e9) * np.random.uniform(0.8, 1.2)
        self.pos = np.random.uniform(
            [self.config["area"][0], self.config["area"][2], 0],
            [self.config["area"][1], self.config["area"][3], 0]
        )
...
class UAVGenAIEnv(gym.Env):
    ...
    def step(self, action):
        D = np.clip(action[:self.num_users], 0, self.Dmax)
        self.uav_pos = np.clip(action[self.num_users:], 
                               [self.area[0], self.area[2], self.z_range[0]],
                               [self.area[1], self.area[3], self.z_range[1]])
        self._update_channel_gains()
        rewards, Qs, Ls, Es = [], [], [], []

        for i, ue in enumerate(self.ues):
            D_i = int(D[i]) if ue.w == 1 else 0
            Q_i = compute_qos(ue, D_i)
            L_i = compute_latency(ue, D_i)
        ...
        reward = sum(rewards) + self.psi if all(Qs[i] <= self.Qreq[i] and Ls[i] <= self.tau for i in range(self.num_users)) else sum(rewards)
        self.time += 1
        ...
        self._update_channel_gains()
        self.state = self._get_state()
        return self.state, reward, done, {"Qs": Qs, "Es": Es, "Ls": Ls}

    def _get_state(self):
        state = []
        for ue in self.ues:
            state.extend([ue.Simg, ue.Sprompt, ue.w, ue.f, ue.g, ue.gUAV_BS, ue.K])
        state.extend(self.uav_pos.tolist())
        return np.array(state, dtype=np.float32)

    def _update_channel_gains(self):
        for ue in self.ues:
            ue.g = compute_channel_gain(ue.pos, self.uav_pos)
            ue.gUAV_BS = compute_channel_gain(self.uav_pos, self.BS_position)
