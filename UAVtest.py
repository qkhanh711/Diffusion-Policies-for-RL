...
def train_agent(env, agent, device, num_episodes=10000, batch_size=64, algo='ql', test = False):
    ...
    # Save reward log
        np.save(f"testUAV/episode_rewards_{algo}_JLGO.npy", np.array(episode_rewards))

    # Save loss log
        with open(f"testUAV/loss_log_{algo}_JLGO.json", "w") as f:
            json.dump(loss_log, f, indent=4)
    if test:
        debug_env(num = 3)

    print(f"Saving reward log to testUAV/episode_rewards_{algo}_JLGO.npy")
...
def run(algo=None, test = False):
    ...
    env.reset()
    # Use observation space dimension instead of _get_state() dimension
    state_dim = env.observation_space.shape[0]
    print(f"State dimension: {state_dim}")
    print(f"Observation space shape: {env.observation_space.shape}")
    action_dim = env.num_ues + 3
    ...
    #         lr_decay=0.99,
    #         lr_maxt=1000,
    #         grad_norm=0.5  # Reduced gradient norm
    #     )
    # elif algo == 'bc':
    ...
    if test:
        ...
        train_agent(env, agent, device, num_episodes=5000, batch_size=64, algo=algo, test = test)
